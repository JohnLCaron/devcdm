...
import dev.ucdm.dataset.geoloc.projection.RotatedPole;
...
class GribIospBuilder {
  ...
  final private GribCollection gribCollection;
  final private GribTables gribTable;

  GribIospBuilder(GribIosp iosp, boolean isGrib1, Logger logger, GribCollection gribCollection,
  ...
  void addGroup(Group.Builder parent, GribCollection.GroupGC group, CollectionType gctype,
      boolean useGroups) {
    Group.Builder g;
    if (useGroups) {
  ...
  private void makeGroup(Group.Builder g, GribCollection.GroupGC group, CollectionType gctype) {
    ...
    List<VariableIndex> vlist = new ArrayList<>(group.variList);
    vlist.sort(Comparator.comparing(vindex -> iosp.makeVariableName(vindex)));
    for (VariableIndex vindex : vlist) {
      try (Formatter dimNames = new Formatter(); Formatter coordinateAtt = new Formatter()) {
        ...
        switch (gctype) {
          ...
          } // PC: Multiple Runtime Unique Time Partition [ntimes]
          case MRUTP, MRUTC -> { // GC: Multiple Runtime Unique Time Collection [ntimes]
          ...
          } // GC: Multiple Runtime Collection [nruns, ntimes] (run, 2D) use Both
          case MRC, TwoD -> { // PC: TwoD time partition [nruns, ntimes]
          ...
          case Best, BestComplete -> { // PC: Best complete time partition [ntimes]
            dimNames.format("%s ", timeDimName);
            coordinateAtt.format("ref%s %s ", timeDimName, timeDimName);
          }
          default -> throw new IllegalStateException("Uknown GribCollection TYpe = " + gctype);
        }
