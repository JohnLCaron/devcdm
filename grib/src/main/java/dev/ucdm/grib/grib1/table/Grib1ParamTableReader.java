...
public class Grib1ParamTableReader {
  ...
  Grib1ParamTableReader(Element paramTableElem) {
    ...
    DssParser p = new DssParser(NcmlReader.ncmlNS);
   ...
   * generated by EcmwfLocalConcepts.
   * <p>
   * The original localConcepts files are located in:
   * <p/>
   * grib/src/main/sources/ecmwfEcCodes/
   * <p/>
   * Since we write the table file that are ultimately read by CDM, the format is controlled and is the following:
   * <p>
  ...
  private Map<Integer, Grib1Parameter> readParameterTableEcmwfEcCodes() throws IOException {
    ...
    try (InputStream is = GribResourceReader.getInputStream(path);
      ...
      while (true) {
        ...
        // unit = WmoUtils.cleanUnit(unit); // fixes some common unit mistakes // jcaron - just use unit as it is
