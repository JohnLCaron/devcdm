...
public class TestNetcdfWriteAndUpdate {
  ...
  public void testWriteAndUpdate() throws Exception {
    ...
    // test some errors
    assertThat(assertThrows(RuntimeException.class, () -> Arrays.factory(ArrayType.SEQUENCE, new int[] {1}, new Object[1]))
    ...
    try (CdmFile ncfile = CdmFiles.open(writerLocation)) {
      // read entire array
    ...
    try (Netcdf3FormatWriter writer = Netcdf3FormatWriter.openExisting(writerLocation).build()) {
      Variable v = writer.findVariable("temperature");
      assertThat(v).isNotNull();
