...
/**
 * Simple example using {@link Netcdf3FormatWriter} to create a new netCDF file corresponding to the following CDL:
 *
 * <pre>
 *  netcdf example {
 *  dimensions:
...
public class TestWriteRecord {
  ...
  public void testNC3WriteWithRecordVariables() throws Exception {
    ...
    try (CdmFile ncfile = CdmFiles.open(filename)) {
      ...
      assertThat(NumericCompare.nearlyEquals(fa.get(0), -109.0f)).isTrue();
      assertThat(NumericCompare.nearlyEquals(fa.get(1), -107.0f)).isTrue();
      assertThat(NumericCompare.nearlyEquals(fa.get(2), -105.0f)).isTrue();
      assertThat(NumericCompare.nearlyEquals(fa.get(3), -103.0f)).isTrue();
