...
import static dev.cdm.core.api.TestUtils.makeDummyGroup;

/** Test {@link dev.cdm.core.api.Variable} */
public class TestVariable {
  ...
  public void testNetcdf() {
    ...
    CdmFile ncfile = CdmFile.builder().setRootGroup(root).setLocation("loca").setId("notFileType").build();

    Variable var = ncfile.findVariable("v");
    assertThat(var).isNotNull();
    assertThat(var.getDatasetLocation()).isEqualTo("loca");
    assertThat(var.getCdmFileTypeId()).isNull();
    assertThat(var.getCdmFile()).isEqualTo(ncfile);
  ...
  public void testEnum() {
    ...
    CdmFile ncfile = CdmFile.builder().setRootGroup(root).build();
  ...
  public void testReadArrayScalarString() throws IOException {
    dev.cdm.array.Array<?> data = Arrays.factory(ArrayType.STRING, new int[] {2}, new String[] {"11", "22"});
    Variable varb =
        Variable.builder().setName("varc").setArrayType(ArrayType.STRING).setSourceData(data).build(makeDummyGroup());
    assertThat(varb.readArray().getScalar()).isEqualTo("11");

    dev.cdm.array.Array<?> arr = varb.readArray();
  ...
  public void testReadArrayScalarChar() throws IOException {
    dev.cdm.array.Array<?> data = Arrays.factory(ArrayType.CHAR, new int[] {3}, new char[] {'1', '2', '3'});
    Variable varb =
        Variable.builder().setName("varc").setArrayType(ArrayType.CHAR).setSourceData(data).build(makeDummyGroup());
    ...
    dev.cdm.array.Array<?> arr = varb.readArray();
    assertThat(arr).isEqualTo(data);
    assertThat(Arrays.makeStringFromChar((dev.cdm.array.Array<Byte>) arr)).isEqualTo("123");
  }
