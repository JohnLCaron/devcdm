...
/** Test {@link dev.cdm.core.api.Attribute} */
public class TestAttribute {
  ...
  public void testSetArrayValues() {
    dev.cdm.array.Array<?> data = Arrays.factory(ArrayType.FLOAT, new int[] {2}, new float[] {3.14f, .001f});
    ...
    dev.cdm.array.Array<?> values = att.getArrayValues();
  ...
  public void testLargeLongValue() {
    Attribute att = new Attribute("name", IospUtils.NC_FILL_INT64); // which is -9223372036854775806L
    long result = att.getNumericValue().longValue(); // returned -9223372036854775808L, before bug fix.
    assertThat(result).isEqualTo(IospUtils.NC_FILL_INT64);
  }
