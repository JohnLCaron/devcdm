...
public class N3iospWriter extends N3iosp implements IospFileWriter {
  ...
  @Override
  public CdmFile create(String filename, Group.Builder rootGroup, int extra, long preallocateSize, boolean largeFile)
      throws IOException {
    ...
    CdmFile.Builder<?> ncfileb = CdmFile.builder().setRootGroup(rootGroup).setLocation(filename);
  ...
  public void openForWriting(String location, Group.Builder rootGroup, CancelTask cancelTask) throws IOException {
    ...
    this.raf = new RandomAccessFile(location, "rw");
    raf.order(RandomAccessFile.BIG_ENDIAN);
    ...
    CdmFile.Builder<?> ncfileb = CdmFile.builder().setRootGroup(rootGroup).setLocation(location);
    this.ncfile = ncfileb.build();
    headerw.setRootGroup(this.ncfile.getRootGroup());
  }

  @Override
  public CdmFile getOutputFile() {
    return this.ncfile;
  }
  ...
  public void writeData(Variable v2, Section section, Array<?> values) throws IOException, InvalidRangeException {
    ...
    if (v2.isUnlimited()) {
      dev.cdm.array.Range firstRange = section.getRange(0);
  ...
  private void writeRecordArrayData(Structure s, Section section, StructureDataArray structureArray)
      throws IOException, InvalidRangeException {
    int countSrcRecnum = 0;
    dev.cdm.array.Range recordRange = section.getRange(0);
    for (int recnum : recordRange) {
      StructureData sdata = structureArray.get(countSrcRecnum);
      writeRecordArrayData(s, recnum, sdata);
      countSrcRecnum++;
    }
  }

  private void writeRecordArrayData(Structure s, int recnum, StructureData sdata)
      throws IOException, InvalidRangeException {
  ...
  private void writeArrayData(Array<?> values, Layout index, ArrayType dataType) throws IOException {
    switch (dataType) {
      case BYTE:
      case CHAR: {
  ...
  @Override
  public int appendStructureData(Structure s, StructureData sdata) {
    return 0;
  }
