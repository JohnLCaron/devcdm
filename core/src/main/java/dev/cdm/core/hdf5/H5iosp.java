...
public class H5iosp extends AbstractIOServiceProvider {
  ...
  void convertHeapArray(ByteBuffer hdf5heap, StructureDataStorageBB storage, int pos, StructureMembers sm)
      throws IOException {
  ...
  @Override
  public Array<?> readArrayData(Variable v2, Section section) throws IOException, InvalidRangeException {
  ...
  Object readArrayOrPrimitive(H5header.Vinfo vinfo, @Nullable Variable v, Layout layout, ArrayType dataType,
    ...
    if (typeInfo.hdfType == 2) { // time
      ...
      for (long time : timeArray) {
        stringData[count++] = Instant.ofEpochMilli(time).toString();
      }
      return stringData;
    }
