...
public class Dimensions {
   ...
   */
  public static dev.cdm.array.Section.Builder makeArraySectionFromDimensions(Iterable<Dimension> dimensions) {
    try {
      dev.cdm.array.Section.Builder builder = dev.cdm.array.Section.builder();
      for (Dimension d : dimensions) {
        int len = d.getLength();
        if (len > 0) {
          builder.appendRange(new dev.cdm.array.Range(d.getShortName(), 0, len - 1));
        } else if (len == 0) {
          builder.appendRange(dev.cdm.array.Range.EMPTY);
        } else {
          Preconditions.checkArgument(d.isVariableLength());
          builder.appendRange(dev.cdm.array.Range.VLEN);
        }
      }
      return builder;
