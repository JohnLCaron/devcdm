...
public class Attribute {
  ...
  public Attribute(String name, @Nullable String val) {
    ...
    this.name = CdmFiles.makeValidCdmObjectName(name);
  ...
  @Nullable
  public dev.cdm.array.Array<?> getArrayValues() {
    if (svalue != null) {
  ...
  void writeCDL(Formatter f, boolean strict, String parentname) {
    ...
    if (strict && parentname != null) {
      f.format(CdmFiles.makeValidCDLName(parentname));
    }
    f.format(":");
    f.format("%s", strict ? CdmFiles.makeValidCDLName(getShortName()) : getShortName());
