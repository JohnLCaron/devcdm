...
public class Variable implements ProxyReader, Comparable<Variable> {
  ...
  /** Get the CdmFile that this variable is contained in. May be null when the Variable is self contained. */
  @Nullable
  ...
  public Variable section(Section subsection) throws InvalidRangeException {
    ...
    Builder<?> sectionV = this.toBuilder(); // subclasses override toBuilder()
  ...
  public Variable slice(int dim, int value) throws InvalidRangeException {
    ...
    // create a copy of this variable with a proxy reader
    Builder<?> sliceV = this.toBuilder(); // subclasses override toBuilder()
  ...
  public void getNameAndDimensions(Formatter buf, boolean useFullName, boolean strict) {
    ...
    if (strict) {
      name = CdmFiles.makeValidCDLName(getShortName());
    }
    buf.format("%s", name);
  ...
  protected void writeCDL(Formatter buf, Indent indent, boolean useFullName, boolean strict) {
    ...
    if (getArrayType().isEnum()) {
      if (enumTypedef == null)
        ...
        buf.format("enum %s", CdmFiles.makeValidCDLName(enumTypedef.getShortName()));
    } else
  ...
  @Nullable
  protected final CdmFile ncfile; // Physical container for this Variable where the I/O happens.
  ...
  public static abstract class Builder<T extends Builder<T>> {
    ...
    public T setNcfile(CdmFile ncfile) {
      this.ncfile = ncfile;
      return self();
    }
