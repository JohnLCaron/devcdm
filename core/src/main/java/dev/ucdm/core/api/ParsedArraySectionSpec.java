...
public class ParsedArraySectionSpec {
   ...
   * @throws dev.ucdm.array.InvalidRangeException if section does not match variable shape
   */
  public static ParsedArraySectionSpec parseVariableSection(CdmFile ncfile, String variableSection)
      throws dev.ucdm.array.InvalidRangeException {
  ...
  private static Section makeSpec(StringBuilder sb, Variable v, Section orgSection) {
    ...
    if (!v.isVariableLength() && !v.isScalar()) { // sequences cant be sectioned
      ...
      for (int count = 0; count < v.getRank(); count++) {
        dev.ucdm.array.Range r = vsection.getRange(count);
        if (r == null)
          r = new dev.ucdm.array.Range(v.getDimension(count).getLength());
        if (count > 0)
          sb.append(", ");
        sb.append(r);
      }
