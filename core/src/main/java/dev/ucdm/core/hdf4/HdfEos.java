...
public class HdfEos {
  ...
  /**
   * Amend the given CdmFile with metadata from HDF-EOS structMetadata.
  ...
  private void amendFromODL(Group.Builder rootg, String structMetadata) {
    ...
    if (swathStructure != null) {
      ...
      for (Element elemSwath : swaths) {
        ...
        String swathName = CdmFiles.makeValidCdmObjectName(swathNameElem.getText().trim());
    ...
    if (gridStructure != null) {
      ...
      for (Element elemGrid : grids) {
        ...
        String gridName = CdmFiles.makeValidCdmObjectName(gridNameElem.getText().trim());
  ...
  private FeatureType amendSwath(Element swathElem, Group.Builder parent) {
    ...
    for (Element elem : dims) {
      ...
      name = CdmFiles.makeValidCdmObjectName(name);
    ...
    for (Element elem : dimMaps) {
      ...
      geoDimName = CdmFiles.makeValidCdmObjectName(geoDimName);
      String dataDimName = elem.getChild("DataDimension").getText().trim();
      dataDimName = CdmFiles.makeValidCdmObjectName(dataDimName);
    ...
    if (geoFieldsG != null) {
      ...
      for (Element elem : floc.getChildren()) {
        ...
        if ((latAxis != null) && (lonAxis != null)) {
          ...
          // TODO int xyDomainSize = CoordSystemBuilder.countDomainSize(latAxis, lonAxis);
          int xyDomainSize = 42;
    ...
    if (dataG != null) {
      ...
      for (Element elem : vars) {
        ...
        String varname = CdmFiles.makeValidCdmObjectName(dataFieldNameElem.getText().trim());
  ...
  private FeatureType amendGrid(Element gridElem, Group.Builder parentGroupBuilder, String location) {
    ...
    if (dataG != null) {
      ...
      for (Element elem : vars) {
        ...
        varname = CdmFiles.makeValidCdmObjectName(varname);
      ...
      if (isLatLon) {
        for (Variable.Builder<?> v : dataG.vbuilders) {
          // TODO if (CoordSystemBuilder.isCoordinateVariable(v)) {
  ...
  private void setSharedDimensions(Group.Builder group, Variable.Builder<?> v, List<Element> values,
    ...
    for (int i = 0; i < values.size(); i++) {
      ...
      String dimName = value.getText().trim();
      dimName = CdmFiles.makeValidCdmObjectName(dimName);
