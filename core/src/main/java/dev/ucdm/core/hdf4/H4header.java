...
public class H4header implements HdfHeaderIF {
  ...
  private void adjustDimensions() {
    ...
    for (Dimension dim : List.copyOf(root.dimensions)) {
  ...
  // TODO test this
  private Structure makeChunkVariable(CdmFile ncfile, TagVH vh) {
    ...
    for (int i = 0; i < group.nelems; i++) {
      ...
      if (tag.code == 1965) {
        ...
        if (vg.className.startsWith("Dim") || vg.className.startsWith("UDim")) {
          String dimName = CdmFiles.makeValidCdmObjectName(vg.name);
  ...
  class Vinfo implements Comparable<Vinfo> {
    ...
    void setLayoutInfo(CdmFile ncfile) throws IOException {
      if (data == null)
        return;
    ...
    List<DataChunk> readChunks(CdmFile ncfile) throws IOException {
  ...
  private class SpecialChunked {
    ...
    private List<DataChunk> getDataChunks(CdmFile ncfile) throws IOException {
      if (dataChunks == null) {
