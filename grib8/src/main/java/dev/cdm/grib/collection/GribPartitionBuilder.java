...
import dev.cdm.core.inventory.PartitionManager;
import dev.cdm.core.io.Streams;
import dev.cdm.core.inventory.MCollection;
import dev.cdm.core.inventory.MFile;
import dev.cdm.grib.util.GribIndexCache;
import dev.cdm.grib.coord.CalendarDateFactory;
import dev.cdm.grib.coord.Coordinate;
import dev.cdm.grib.coord.CoordinateBuilder;
import dev.cdm.grib.coord.CoordinateEns;
import dev.cdm.grib.coord.CoordinatePartitionUnionizer;
import dev.cdm.grib.coord.CoordinateRuntime;
import dev.cdm.grib.coord.CoordinateSharer;
import dev.cdm.grib.coord.CoordinateSharerBest;
import dev.cdm.grib.coord.CoordinateTime;
import dev.cdm.grib.coord.CoordinateTime2D;
import dev.cdm.grib.coord.CoordinateTimeAbstract;
import dev.cdm.grib.coord.CoordinateTimeIntv;
import dev.cdm.grib.coord.CoordinateVert;
...
abstract class GribPartitionBuilder {
  ...
  boolean createPartitionedIndex(CollectionUpdateType forcePartition, Formatter errlog) throws IOException {
    ...
    for (MCollection dcmp : partitionManager.makePartitions(forcePartition)) {
      dcmp.putAuxInfo(GribConfig.AUX_CONFIG,
          partitionManager.getAuxInfo(GribConfig.AUX_CONFIG));
  ...
  private class GroupPartitions {
    ...
    void makeVariableIndexPartitioned() {
      ...
      Map<GribCollectionMutable.VariableIndex, GribCollectionMutable.VariableIndex> varMap =
          new HashMap<>(2 * resultGroup.variList.size());
  ...
  private GribCollectionMutable.Dataset makeDataset2D(Formatter f) throws IOException {
    GribConfig gribConfig =
        (GribConfig) partitionManager.getAuxInfo(GribConfig.AUX_CONFIG);
    ...
    for (PartitionCollectionMutable.Partition tpp : result.getPartitions()) {
      ...
      try (GribCollectionMutable gc = tpp.makeGribCollection()) {
         ...
         * }
         * }
         * } else if (ds2dp.gctype == GribCollectionImmutable.Type.MRC || ds2dp.gctype ==
         * GribCollectionImmutable.Type.TwoD) {
         * allAre1D = false;
         * }
         */
      ...
      for (int partno = 0; partno < npart; partno++) {
        ...
        for (int varIdx = 0; varIdx < group.variList.size(); varIdx++) {
          ...
          PartitionCollectionMutable.VariableIndexPartitioned vip =
              (PartitionCollectionMutable.VariableIndexPartitioned) resultGroup.findVariableByHash(vi);
  ...
  // writing ncx2
  // The Groups are GribCollection.GroupGC, not GribXCollectionWriter.Group, so we cant inheritit most of the methods
  // from GribXCollectionWriter
   ...
   * }
   */
  private GribCollectionProto.Dataset writeDatasetProto(PartitionCollectionMutable pc, GribCollectionMutable.Dataset ds)
      throws IOException {
   ...
   * }
   */
  private GribCollectionProto.Group writeGroupProto(PartitionCollectionMutable pc, GribCollectionMutable.GroupGC g)
      throws IOException {
    GribCollectionProto.Group.Builder b = GribCollectionProto.Group.newBuilder();

    b.setGds(
        GribCollectionWriter.writeGdsProto(g.horizCoordSys.getRawGds(), g.horizCoordSys.getPredefinedGridDefinition()));
   ...
   * }
   */
  private GribCollectionProto.PartitionVariable writePartitionVariableProto(int partno, int groupno, int varno,
      int nrecords, int ndups, int nmissing) {
   ...
   * }
   */
  private GribCollectionProto.Partition writePartitionProto(PartitionCollectionMutable pc,
      PartitionCollectionMutable.Partition p) {
