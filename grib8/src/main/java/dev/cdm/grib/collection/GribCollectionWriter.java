...
import dev.cdm.grib.coord.EnsCoordValue;
import dev.cdm.grib.coord.TimeCoordIntvValue;
import dev.cdm.grib.coord.VertCoordValue;
...
class GribCollectionWriter {
   ...
   * not currently used
   * 
   * protected GribCollectionProto.FcConfig writeConfig(GribConfig config) throws IOException {
   ...
   * 
   * GribConfig.GribConfig gribConfig = config.gribConfig;
   ...
   * for (GribConfig.GribIntvFilterParam intvFilter : gribConfig.intvFilter.filterList) {
   ...
   * }
   * 
   * protected GribConfig readConfig(boolean isGrib1, GribCollectionProto.FcConfig pconfig) throws
   * IOException {
   * 
   ...
   * config.ptype = GribConfig.PartitionType.valueOf(pconfig.getPartitionType());
   ...
   * config.gribConfig.intvFilter = new GribConfig.GribIntvFilter();
   ...
   * config.gribConfig.intvFilter.filterList.add(new GribConfig.GribIntvFilterParam(pi.getVariableId(),
   * pi.getIntvLength(), prob));
   * }
   * }
   * 
   * if ( pconfig.getTimeUnitConvertCount() > 0) {
   * config.gribConfig.tuc = new GribConfig.TimeUnitConverterHash();
