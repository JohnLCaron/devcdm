...
import dev.cdm.core.inventory.CollectionUpdateType;
import dev.cdm.core.inventory.IndexReader;
import dev.cdm.core.inventory.MCollection;
import dev.cdm.core.inventory.MFile;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import dev.cdm.grib.util.GribIndexCache;
import dev.cdm.grib.grib1.Grib1RecordScanner;
import dev.cdm.grib.grib2.Grib2RecordScanner;
...
public class GribCdmIndex implements IndexReader {
  ...
  public static File getTopIndexFileFromConfig(GribConfig config) {
   ...
   *
   * @param config use this GribConfig
   * @return index File
   */
  private static File makeTopIndexFileFromConfig(GribConfig config) {
   ...
   * public static GribCollectionImmutable openCdmIndex(String indexFilename, GribConfig config, Logger
   * logger) {
  ...
  // open GribCollectionImmutable from an existing index file. return null on failure
  @Nullable
  public static GribCollectionImmutable openCdmIndex(String indexFilename, GribConfig config,
  ...
  // open GribCollectionImmutable from an existing index file. return null on failure
  @Nullable
  public static GribCollectionMutable openMutableGCFromIndex(String indexFilename, GribConfig config,
   ...
   */
  public static boolean updateGribCollection(GribConfig config, CollectionUpdateType updateType,
    ...
    if (config.ptype == GribConfig.PartitionType.none
        || config.ptype == GribConfig.PartitionType.all) {

      try (CollectionAbstract dcm = new CollectionPathMatcher(config, specp, logger)) {
        changed =
            updateGribCollection(isGrib1, dcm, updateType, GribConfig.PartitionType.none, logger, errlog);
      }

    } else if (config.ptype == GribConfig.PartitionType.timePeriod) {
      ...
      if (specp.wantSubdirs()) { // its a partition
        ...
        try (DirectoryPartition dpart =
          ...
          dpart.putAuxInfo(GribConfig.AUX_CONFIG, config);
  ...
  public static boolean updateGribCollection(boolean isGrib1, MCollection dcm, CollectionUpdateType updateType,
      GribConfig.PartitionType ptype, Logger logger, Formatter errlog) throws IOException {
  ...
  private static boolean updateTimePartition(boolean isGrib1, TimePartition tp, CollectionUpdateType updateType,
    ...
    for (MCollection part : tp.makePartitions(updateType)) {
      try {
        updateGribCollection(isGrib1, part, updateType, GribConfig.PartitionType.timePeriod, logger,
            errlog);
  ...
  private static boolean updateDirectoryCollectionRecurse(boolean isGrib1, DirectoryPartition dpart,
      GribConfig config, CollectionUpdateType updateType, Logger logger) throws IOException {
    ...
    if (updateType != CollectionUpdateType.testIndexOnly) { // skip children on testIndexOnly
      for (MCollection part : dpart.makePartitions(updateType)) {
        part.putAuxInfo(GribConfig.AUX_CONFIG, config);
        try {
  ...
  private static boolean updateLeafCollection(boolean isGrib1, GribConfig config,
    ...
    if (config.ptype == GribConfig.PartitionType.file) {
      ...
      try (DirectoryCollection dcm =
        ...
        dcm.putAuxInfo(GribConfig.AUX_CONFIG, config);
        ...
        boolean changed = updateGribCollection(isGrib1, dcm, updateType,
            GribConfig.PartitionType.directory, logger, errlog);
   ...
   */
  private static boolean updateFilePartition(boolean isGrib1, GribConfig config,
      CollectionUpdateType updateType, boolean isTop, Logger logger, Path dirPath) throws IOException {
    long start = System.currentTimeMillis();
    Formatter errlog = new Formatter();
    CollectionSpecParser specp = config.getCollectionSpecParser(errlog);
    ...
    try (FilePartition partition = new FilePartition(config.collectionName, dirPath, isTop, config.olderThan, logger)) {
      partition.putAuxInfo(GribConfig.AUX_CONFIG, config);
      if (specp.getFilter() != null)
        ...
        partition.iterateOverMFileCollection(mfile -> {
          ...
          try {
            boolean changed = updateGribCollection(isGrib1, part, updateType,
                GribConfig.PartitionType.file, logger, errlog);
            if (changed)
              anyChange.set(true);
  ...
  public static boolean makeIndex(GribConfig config, Formatter errlog, Path topPath) throws IOException {
    return false;
    /*
   ...
   */
  /*
   * public static boolean rewriteDirectoryCollection(final GribConfig config,
   ...
   * dirCollection.putAuxInfo(GribConfig.AUX_GRIB_CONFIG, config.gribConfig);
   ...
   */
  public static GribCollectionImmutable openGribCollection(GribConfig config,
  ...
  public static GribCollectionImmutable openGribCollectionFromRaf(RandomAccessFile raf, GribConfig config,
  ...
  private static GribCollectionImmutable openGribCollectionFromDataFile(boolean isGrib1, RandomAccessFile dataRaf,
      GribConfig config, CollectionUpdateType updateType, Formatter errlog, org.slf4j.Logger logger)
      throws IOException {
  ...
  public static GribCollectionImmutable openGribCollectionFromDataFile(boolean isGrib1, MFile mfile,
      CollectionUpdateType updateType, GribConfig config, Formatter errlog, org.slf4j.Logger logger)
      throws IOException {
    ...
    if (isGrib1) {
      ...
      Grib1CollectionBuilder builder = new Grib1CollectionBuilder(dcm.getCollectionName(), dcm, logger);
      boolean changed =
          (builder.updateNeeded(updateType) && builder.createIndex(GribConfig.PartitionType.file, errlog));
    } else {
      Grib2CollectionBuilder builder = new Grib2CollectionBuilder(dcm.getCollectionName(), dcm, logger);
      boolean changed =
          (builder.updateNeeded(updateType) && builder.createIndex(GribConfig.PartitionType.file, errlog));
  ...
  public static GribCollectionImmutable openGribCollectionFromIndexFile(RandomAccessFile indexRaf,
      GribConfig config, org.slf4j.Logger logger) throws IOException {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      ...
      Formatter errlog = new Formatter();
      try {
        GribConfigBuilder reader = new GribConfigBuilder(errlog);
        String location = cmdLine.inputFile.getAbsolutePath();
        GribConfig config = reader.readConfigFromFile(location);
