...
abstract class GribCollectionBuilderFromIndex {
  ...
  GribCollectionBuilderFromIndex(GribCollectionMutable gc, GribConfig config, org.slf4j.Logger logger) {
    this.logger = logger;
  ...
  protected boolean readIndex(RandomAccessFile raf) {
    ...
    try {
      ...
      for (int i = 0; i < n; i++) {
        dev.cdm.grib.collection.GribCollectionProto.MFile mf = proto.getMfiles(i);
