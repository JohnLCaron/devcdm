...
public class TestGribSize {
  ...
  private long doGrib2(boolean showDetail, String filename) {
    ...
    try (RandomAccessFile raf = new RandomAccessFile(filename, "r")) {
      ...
      while (reader.hasNext()) {
        ...
        if (nrecords % 1000 == 0)
          System.out.printf("%d nsecs / record%n", gribReadTime / 1000);
      }
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  private void makeSummaryHeader() {
    ...
    f.format("%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s%n", "file", "edition", "fileSize", "nPoints",
        "nDataPoints", "nrecords", "gribFileSize", "dataSize", "bitMapSize", "ratio", "dataR", "float size",
        "float/grib");
  ...
  private void summaryReport(String filename, int edition, long fileSize, long nPoints, long nDataPoints, long nrecords,
      long msgSize, long dataSize, long bitmapSize) {
    ...
    f.format("%s, %d, %d, %d, %d, %d, %d, %d, %d, %f, %f, %d, %f%n", filename, edition, fileSize, nPoints, nDataPoints,
        nrecords, msgSize, dataSize, bitmapSize, ratio, dataR, floatSize, floatRatio);
