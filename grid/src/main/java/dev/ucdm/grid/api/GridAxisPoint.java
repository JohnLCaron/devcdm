...
import dev.ucdm.grid.internal.SubsetHelpers;
import dev.ucdm.grid.internal.SubsetPointHelper;
...
public class GridAxisPoint extends GridAxis<Number> implements Iterable<Number> {
  ...
  public Optional<GridAxisPoint> subset(GridSubset params, Formatter errlog) {
    ...
    Builder<?> builder = helper.subsetBuilder(params, errlog);
    if (builder == null) {
  ...
  public CoordInterval getCoordInterval(int index) {
    return new CoordInterval(getCoordEdge1(index), getCoordEdge2(index));
  }
  ...
  public int binarySearch(double want) {
    if (isRegular()) {
      return SubsetHelpers.findCoordElement(this, want, true);
    } else {
  ...
  public static abstract class Builder<T extends Builder<T>> extends GridAxis.Builder<T> {
    ...
    private RangeIterator rangeIter; // for subset, tracks the indexes in the original
    ...
    public T setRangeIterator(RangeIterator range) {
      Preconditions.checkNotNull(range);
      this.rangeIter = range;
      return self();
    }
