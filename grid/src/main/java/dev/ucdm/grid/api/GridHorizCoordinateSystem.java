...
import dev.ucdm.grid.internal.CylindricalCoord;
...
public class GridHorizCoordinateSystem {
  ...
  public LatLonPoint getLatLon(int xindex, int yindex) {
    ...
    return isLatLon() ? new LatLonPoint(y, x) : makeLatLon(x, y);
  }
  ...
  private LatLonRect calcLatLonBoundingBox() {
    if (isLatLon()) {
      ...
      LatLonPoint llpt = new LatLonPoint(Math.min(y1, y2), Math.min(x1, x2));
