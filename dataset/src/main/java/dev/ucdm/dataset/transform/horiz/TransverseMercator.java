...
public class TransverseMercator extends AbstractProjectionCT implements ProjectionBuilder {
  ...
  public Projection makeProjection(AttributeContainer ctv, String geoCoordinateUnits) {
    ...
    if (!Double.isNaN(semi_major_axis) && (!Double.isNaN(semi_minor_axis) || inverse_flattening != 0.0)) {
      ...
      proj = new dev.ucdm.dataset.geoloc.projection.proj4.TransverseMercatorProjection(earth, lon0, lat0, scale,
          false_easting, false_northing);
    } else {
      proj = new dev.ucdm.dataset.geoloc.projection.TransverseMercator(lat0, lon0, scale, false_easting, false_northing,
          earth_radius);
    }
    return proj;
  }
  ...
  public Class<? extends Projection> getProjectionClass() {
    return dev.ucdm.dataset.geoloc.projection.TransverseMercator.class;
  }
}
