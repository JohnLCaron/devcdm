 ...
 */
package dev.cdm.dataset.geoloc.projection.proj4;
...
public class PolyconicProjection extends AbstractProjection {
  ...
  public boolean crossSeam(ProjectionPoint pt1, ProjectionPoint pt2) {
    ...
    return (pt1.x() * pt2.x() < 0) && (Math.abs(pt1.x() - pt2.x()) > 20000.0);
  ...
  public ProjectionPoint latLonToProj(LatLonPoint latlon) {
    double fromLat = Math.toRadians(latlon.latitude());
    double theta = Math.toRadians(latlon.longitude());
    ...
    return ProjectionPoint.create(totalScale * out.x() + falseEasting, totalScale * out.y() + falseNorthing);
  }

  @Override
  public LatLonPoint projToLatLon(ProjectionPoint world) {
    double fromX = (world.x() - falseEasting) / totalScale; // assumes cartesian coords in km
    double fromY = (world.y() - falseNorthing) / totalScale;
    double toLat, toLon;
    ...
    if (pp.x() < -Math.PI) {
      toLon = -Math.PI;
    } else if (pp.x() > Math.PI) {
      toLon = Math.PI;
    }
    if (projectionLongitude != 0 && !Double.isNaN(pp.x())) {
