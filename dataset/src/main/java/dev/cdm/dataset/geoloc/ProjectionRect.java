...
public class ProjectionRect {
  ...
  public ProjectionRect(ProjectionPoint corner1, ProjectionPoint corner2) {
    this(corner1.x(), corner1.y(), corner2.x(), corner2.y());
  }
  ...
  public boolean contains(ProjectionPoint point) {
    return DoubleMath.fuzzyCompare(point.x(), getMinX(), 1e-6) >= 0
        && DoubleMath.fuzzyCompare(point.x(), getMaxX(), 1e-6) <= 0
        && DoubleMath.fuzzyCompare(point.y(), getMinY(), 1e-6) >= 0
        && DoubleMath.fuzzyCompare(point.y(), getMaxY(), 1e-6) <= 0;
  }
  ...
  public ProjectionPoint getLowerRightPoint() {
    return ProjectionPoint.create(getMaxPoint().x(), getMinPoint().y());
  }
  ...
  public ProjectionPoint getUpperLeftPoint() {
    return ProjectionPoint.create(getMinPoint().x(), getMaxPoint().y());
  }
  ...
  /**
   * Returns the result of {@link #nearlyEquals(ProjectionRect, double)}, with {@link NumericCompare#defaultMaxRelativeDiffFloat}.
   */
