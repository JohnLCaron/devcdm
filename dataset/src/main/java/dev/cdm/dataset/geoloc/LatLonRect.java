...
public class LatLonRect {
  ...
  public double getCenterLon() {
    double lon0 = (upperRight.longitude() + lowerLeft.longitude()) / 2;
    if (crossDateline) {
      lon0 -= 180;
    }
    return lon0;
  ...
  /**
   * Returns the result of {@link #nearlyEquals(LatLonRect, double)}, with {@link NumericCompare#defaultMaxRelativeDiffFloat}.
   */
  public boolean nearlyEquals(LatLonRect other) {
    return nearlyEquals(other, NumericCompare.defaultMaxRelativeDiffFloat);
  }
   ...
   * @param maxRelDiff the maximum {@link NumericCompare#relativeDifference relative difference} that two corners may have.
  ...
  public boolean contains(double lat, double lon) {
    // check lat first
    double eps = 1.0e-9;
    if ((lat + eps < lowerLeft.latitude()) || (lat - eps > upperRight.latitude())) {
      return false;
    }
    ...
    if (crossDateline) {
      ...
      return ((lon >= lowerLeft.longitude()) || (lon <= upperRight.longitude()));
    } else {
  ...
  public String toString() {
    return String.format("%f, %f, %f, %f", lowerLeft.latitude(), lowerLeft.longitude(), getHeight(), getWidth());
  }

  /** Convert to a mutable Builder */
  public Builder toBuilder() {
    return builder(this.lowerLeft, this.upperRight.latitude() - this.lowerLeft.longitude(), this.width);
  }
  ...
  public static Builder builder(LatLonPoint left, LatLonPoint right) {
    double width = right.longitude() - left.longitude();
    while (width < 0.0) {
      width += 360;
    }
    if (width == 0.0) {
    ...
    return builder(left, right.latitude() - left.latitude(), width);
  }
  ...
  public static class Builder {
    ...
    public Builder extend(LatLonPoint latlon) {
      return extend(latlon.latitude(), latlon.longitude());
    }
