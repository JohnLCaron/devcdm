...
public class EquidistantAzimuthalProjection extends AbstractProjection {
  ...
  public ProjectionPoint latLonToProj(LatLonPoint latlon) {
    double lam = Math.toRadians(latlon.longitude() - lon0);
    double phi = Math.toRadians(latlon.latitude());
    double toX = 0, toY = 0;
  ...
  public LatLonPoint projToLatLon(ProjectionPoint ppt) {
    double x = (ppt.x() - falseEasting) / totalScale; // assumes cartesion coords in km
    double y = (ppt.y() - falseNorthing) / totalScale;
    double toLat, toLon;
