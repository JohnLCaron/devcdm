...
public class NUWGConvention extends CoordSystemBuilderOld {
  ...
  NUWGConvention(CdmDatasetCS.Builder<?> datasetBuilder) {
  ...
  private boolean makeCoordinateAxis(Variable.Builder<?> ncvar0, Variable.Builder<?> ncvar1, Dimension dim) {
    ...
    try {
      ...
      Array<Double> data0 = Arrays.toDouble(ds0.orgVar.readArray());
      Array<Double> data1 = Arrays.toDouble(ds1.orgVar.readArray());
  ...
  private class Grib1 {
    ...
    private ProjectionCTV makeLCProjection() throws NoSuchElementException {
      ...
      ProjectionPoint start = lc.latLonToProj(new LatLonPoint(la1, lo1));
      if (debug)
    ...
    private ProjectionCTV makePSProjection() throws NoSuchElementException {
      ...
      // we have to project in order to find the origin
      ProjectionPoint start = ps.latLonToProj(new LatLonPoint(la1, lo1));
      if (debug)
        System.out.println("start at proj coord " + start);
      startx = start.x();
      starty = start.y();
