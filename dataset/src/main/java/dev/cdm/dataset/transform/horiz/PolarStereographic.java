...
public class PolarStereographic extends AbstractProjectionCT implements ProjectionBuilder {
  ...
  public Projection makeProjection(AttributeContainer ctv, String geoCoordinateUnits) {
    ...
    if (!Double.isNaN(semi_major_axis) && (!Double.isNaN(semi_minor_axis) || inverse_flattening != 0.0)) {
      ...
      proj = new dev.cdm.dataset.geoloc.projection.proj4.StereographicAzimuthalProjection(lat0, lon0, scale, latD,
          false_easting, false_northing, earth);
    } else {
      proj = new dev.cdm.dataset.geoloc.projection.Stereographic(lat0, lon0, scale, false_easting, false_northing,
          earth_radius);
    }
    return proj;
  }
