...
public class LambertConformalConic extends AbstractProjectionCT implements ProjectionBuilder {
  ...
  public Projection makeProjection(AttributeContainer ctv, String geoCoordinateUnits) {
    ...
    if (!Double.isNaN(semi_major_axis) && (!Double.isNaN(semi_minor_axis) || inverse_flattening != 0.0)) {
      ...
      proj = new dev.cdm.dataset.geoloc.projection.proj4.LambertConformalConicEllipse(lat0, lon0, pars[0], pars[1],
          false_easting, false_northing, earth);

    } else {
      proj = new dev.cdm.dataset.geoloc.projection.LambertConformal(lat0, lon0, pars[0], pars[1], false_easting,
  ...
  public Class<? extends Projection> getProjectionClass() {
    return dev.cdm.dataset.geoloc.projection.LambertConformal.class;
  }
}
