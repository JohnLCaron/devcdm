...
public class TestStandardVar {
  ...
  public void testWriteStandardVar() throws Exception {
    Netcdf3FormatWriter.Builder<?> writerb = Netcdf3FormatWriter.builder().setLocation(filename).setFill(false);
  ...
  private CdmFile ncfileRead;
  private CdmDataset dsRead;

  @Test
  public void testReadStandardVar() throws Exception {
    ncfileRead = CdmFiles.open(filename);
    dsRead = CdmDatasets.openDataset(filename);
  ...
  public void testEnhanceDefer() throws IOException {
    ...
    try (CdmDataset ncd =
        CdmDatasets.openDataset(durl, EnumSet.of(CdmDataset.Enhance.ApplyScaleOffset), -1, null, null)) {
      try (CdmDataset ncdefer = CdmDatasets.openDataset(durl, null, -1, null, null)) {
        ...
        dev.cdm.array.Array<?> enhancedData = enhancedVar.readArray();
        dev.cdm.array.Array<?> deferredData = deferVar.readArray();

        logger.debug("Enhanced = {}", PrintArray.printArray(enhancedData));
        ...
        dev.cdm.array.Array<?> processedData = enhancedVar.scaleMissingUnsignedProxy().applyScaleOffset(deferredData);
