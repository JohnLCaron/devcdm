...
import dev.cdm.array.IsMissingEvaluator;
...
public class TestScaleOffsetMissingUnsigned {
  ...
  public void testWrite() throws Exception {
    ...
    try (CdmFile ncfileRead = CdmFiles.open(filename)) {
    ...
    Set<Enhance> enhance = new HashSet<>(CdmDataset.getDefaultEnhanceMode());
    enhance.remove(Enhance.ConvertMissing);
    try (CdmDataset ncd = CdmDatasets.openDataset(filename, enhance, null)) {
  ...
  private void doSubset(String filename) throws IOException, InvalidRangeException {
    ...
    try (CdmDataset ncd = CdmDatasets.openDataset(filename)) {
      ...
      logger.debug(PrintArray.printArray(readEnhanced));

      Variable sec = vs.section(s);
      Array<?> readSection = sec.readArray();
      logger.debug(PrintArray.printArray(readSection));
  ...
  public void testScaledFillValue() throws Exception {
    ...
    try (CdmDataset ncd = CdmDatasets.openDataset(testResource.getAbsolutePath(), true, null)) {
      ...
      assertThat(NumericCompare.nearlyEquals(expectedFillValue, actualFillValue)).isTrue();
    ...
    Set<Enhance> enhance = new HashSet<>(CdmDataset.getDefaultEnhanceMode());
    enhance.remove(Enhance.ConvertMissing);
    try (CdmDataset ncd = CdmDatasets.openDataset(testResource.getAbsolutePath(), enhance, null)) {
      VariableDS fooVar = (VariableDS) ncd.findVariable("foo");
      assertThat(fooVar).isNotNull();
      ...
      double actualFillValue = proxy.getFillValue();
      assertThat(NumericCompare.nearlyEquals(actualFillValue, fooValWithoutNaNs)).isTrue();
  ...
  public void testScaleMissingFloatingPointComparisons() throws Exception {
    ...
    try (CdmDataset ncd = CdmDatasets.openDataset(testResource.getAbsolutePath(), true, null)) {
      ...
      // its products with nearlyEquals() using the default NumericCompare.defaultMaxRelativeDiffDouble.
      assertThat(NumericCompare.nearlyEquals(0, proxy.getValidMin(), NumericCompare.defaultMaxRelativeDiffFloat)).isTrue();
      assertThat(NumericCompare.nearlyEquals(1, proxy.getValidMax(), NumericCompare.defaultMaxRelativeDiffFloat)).isTrue();
      ...
      assertThat(NumericCompare.nearlyEquals(0.00, fooVals.get(1), NumericCompare.defaultMaxRelativeDiffFloat)).isTrue();
      ...
      assertThat(NumericCompare.nearlyEquals(1.00, fooVals.get(2), NumericCompare.defaultMaxRelativeDiffFloat)).isTrue();
      ...
      assertThat(NumericCompare.nearlyEquals(Double.NaN, fooVals.get(3), NumericCompare.defaultMaxRelativeDiffFloat)).isTrue();
  ...
  public void testScaleMissingFloatingPointComparisonsNoConvertMissing() throws Exception {
    ...
    try (CdmDataset ncd = CdmDatasets.openDataset(testResource.getAbsolutePath(), enhance, null)) {
      ...
      assertThat(fooVals.get(0)).isWithin(NumericCompare.defaultMaxRelativeDiffFloat).of(-0.01f);
      assertThat(proxy.isFillValue(-0.01)).isTrue();
      assertThat(proxy.isMissingValue(-0.01)).isTrue();
      assertThat(proxy.isInvalidData(-0.01)).isTrue();
      assertThat(proxy.isMissing(-0.01)).isTrue();
      ...
      assertThat(NumericCompare.nearlyEquals(1.01, fooVals.get(3), NumericCompare.defaultMaxRelativeDiffFloat)).isTrue();
      assertThat(proxy.isMissingValue(1.01)).isTrue();
      assertThat(proxy.isInvalidData(1.01)).isTrue();
      assertThat(fooVar.isMissing(1.01)).isTrue();
    }
  ...
  public void testMissingUnsigned() throws Exception {
    ...
    try (CdmDataset ncd = CdmDatasets.openDataset(testResource.getAbsolutePath(), true, null)) {
      ...
      assertThat(NumericCompare.nearlyEquals(150, proxy.getValidMin())).isTrue();

      // Packed valid_min == -6. Interpreting bit pattern as unsigned, we get 250.
      assertThat(NumericCompare.nearlyEquals(250, proxy.getValidMax())).isTrue();

      // Packed _FillValue and missing_value are -1. Interpreting bit pattern as unsigned, we get 255.
      assertThat(NumericCompare.nearlyEquals(255, proxy.getFillValue())).isTrue();
      assertThat(NumericCompare.nearlyEquals(255, proxy.getMissingValues()[0])).isTrue();
  ...
  public void testNegativeScaleOffsetValidRangeUnsigned() throws Exception {
    File testResource = new File(getClass().getResource("testScaleOffsetMissingUnsigned.ncml").toURI());
    float fpTol = NumericCompare.defaultMaxRelativeDiffFloat;
  ...
  public void testScaleValidRange() throws Exception {
    ...
    try (CdmDataset ncd = CdmDatasets.openDataset(testResource.getAbsolutePath(), true, null)) {
      ...
      assertThat(NumericCompare.nearlyEquals(9.9f, (float) proxy.getValidMin())).isTrue();
      assertThat(NumericCompare.nearlyEquals(10.1f, (float) proxy.getValidMax())).isTrue();
  ...
  public void testUnpackedValidRange() throws Exception {
    ...
    try (CdmDataset ncd = CdmDatasets.openDataset(location, enhanceMode, -1, null, null)) {
