...
public class TestUtm {
  ...
  void doOne(double x, double y, int zone, boolean isNorth) {
    ...
    System.out.println("   lat=" + latlon.latitude() + " lon=" + latlon.longitude());
      ...
      for (int i = 0; i < NPTS; i++) {
        ...
        if (checkit) {
          ...
          if (check_km("x", from[0][i], endP.getX()) || show)
            System.out.println("   x=" + from[0][i] + " y=" + from[1][i] + " lat=" + latlon.latitude() + " lon="
                + latlon.longitude());
        }
      }
    }
  ...
  public void testStuff() {
    ...
    System.out.printf("%15.12f %15.12f%n", ll.latitude(), ll.longitude());
    assertThat(NumericCompare.nearlyEquals(ll.longitude(), -80.21802662821469, 1.0e-8)).isTrue();
    assertThat(NumericCompare.nearlyEquals(ll.latitude(), 26.685132668190793, 1.0e-8)).isTrue();
  }
