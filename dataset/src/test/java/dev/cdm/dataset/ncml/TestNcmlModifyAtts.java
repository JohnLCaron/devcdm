...
public class TestNcmlModifyAtts {
  private static CdmFile ncfile = null;

  @BeforeAll
  public static void setUp() throws IOException {
    String filename = "file:" + TestNcmlRead.topDir + "modifyAtts.ncml";
  ...
  public void testReadCoordvar() throws IOException {
    ...
    assertThat(NumericCompare.nearlyEquals(dataI.next(), 41.0)).isTrue();
    assertThat(NumericCompare.nearlyEquals(dataI.next(), 40.0)).isTrue();
    assertThat(NumericCompare.nearlyEquals(dataI.next(), 39.0)).isTrue();
  }
  ...
  public void testReadData2() throws IOException {
    ...
    assertThat(NumericCompare.nearlyEquals(dataI.next(), 1.0)).isTrue();
    assertThat(NumericCompare.nearlyEquals(dataI.next(), 2.0)).isTrue();
    assertThat(NumericCompare.nearlyEquals(dataI.next(), 3.0)).isTrue();
    assertThat(NumericCompare.nearlyEquals(dataI.next(), 4.0)).isTrue();
    assertThat(NumericCompare.nearlyEquals(dataI.next(), 2.0)).isTrue();
  }
}
