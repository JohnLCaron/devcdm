...
public class TestCoordinateAxis1DBuilder {
  ...
  public void testFromVariableDS() throws IOException {
    ...
    dev.ucdm.array.Array<?> data = axis.readArray();
    assertThat(data.getArrayType()).isEqualTo(ArrayType.FLOAT);
    assertThat(data.length()).isEqualTo(7);
    dev.ucdm.array.Array<Float> fdata = (Array<Float>) data;
  ...
  public void testFromVariableDSwithData() throws IOException {
    ...
    VariableDS.Builder<?> vdsBuilder = VariableDS.builder().setName("name").setArrayType(ArrayType.FLOAT)
        .setUnits("days since 2001-01-01:00:00").setDesc("desc").setEnhanceMode(CdmDataset.getEnhanceAll())
