 ...
 */
package dev.ucdm.dataset.testutil;
...
public class CompareCdmFiles {
  ...
  public static class Netcdf4ObjectFilter implements ObjFilter {
    ...
    public boolean attCheckOk(Attribute att) {
      ...
      if (name.equals(IospUtils.HDF5_DIMENSION_LIST))
        return false;
      if (name.equals(IospUtils.HDF5_DIMENSION_SCALE))
        return false;
      if (name.equals(IospUtils.HDF5_DIMENSION_LABELS))
        return false;
      ...
      // hidden by nc4
      if (name.equals(IospUtils.NETCDF4_DIMID))
        return false; // preserve the order of the dimensions
      if (name.equals(IospUtils.NETCDF4_COORDINATES))
        return false; // ??
      if (name.equals(IospUtils.NETCDF4_STRICT))
        return false;
  ...
  public boolean compare(CdmFile org, CdmFile copy) {
    return compare(org, copy, showCompare, showEach, compareData);
  }

  public boolean compare(CdmFile org, CdmFile copy, @Nullable ObjFilter filter) {
    return compare(org, copy, filter, showCompare, showEach, compareData);
  }

  private boolean compare(CdmFile org, CdmFile copy, boolean showCompare, boolean showEach, boolean compareData) {
    return compare(org, copy, null, showCompare, showEach, compareData);
  }

  private boolean compare(CdmFile org, CdmFile copy, @Nullable ObjFilter objFilter, boolean showCompare,
  ...
  public boolean compareVariables(CdmFile org, CdmFile copy) {
  ...
  private boolean compareGroups(Group org, Group copy, ObjFilter filter) {
    ...
    // dimensions
    if (filter.checkDimensionsForFile(org.getCdmFile().getLocation())) {
  ...
  private boolean compareVariables(Variable org, Variable copy, ObjFilter filter, boolean compareData,
    ...
    // data !!
    if (compareData) {
      ok &= dev.ucdm.core.util.CompareCdmFiles.compareVariableData(f, org, copy, false);
    }
